...
download_xui(){
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/kali-linux-ir/x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/') || last_version=$(curl -sm8 https://raw.githubusercontent.com/kali-linux-ir/x-ui/main/config/version >/dev/null 2>&1)
        ...
        last_version=$1
        url="https://github.com/kali-linux-ir/x-ui/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz"
...
install_xui() {
    ...
    echo -e "------------------------------------------------------------------------------"
    echo -e "Niduka Akalanka   - https://github.com/kali-linux-ir"
    echo -e "--------------------------------------------------------------------------------"
    show_login_info
    echo -e ""
